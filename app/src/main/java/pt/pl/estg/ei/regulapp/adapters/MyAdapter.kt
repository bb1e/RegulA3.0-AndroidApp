 ...
 */
class MyAdapter(context: Context, estrategias: ArrayList<Estrategia?>, estrategiasPreferidas: ArrayList<Estrategia?>) : ArrayAdapter<Any?>(context, layout.custom_listview, R.id.title,
    ...
    private val thisAdapter: MyAdapter? = null
    public override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        myButton.setOnLikeListener(object : OnLikeListener {
            override fun liked(likeButton: LikeButton?) {
                Toast.makeText(getContext(), "Adicionado ao Favoritos", Toast.LENGTH_SHORT).show()
        ...
        feedbackButton.setOnClickListener {
            val intent: Intent = Intent(context, FeedbackEstrategiaActivity::class.java)
        ...
        db
                ...
                if (task.isSuccessful) {
                    ...
                    if (!(criancas?.isEmpty()!! || criancas == null)) {
                        val crianca: Crianca? = criancas.get(0)
                        Log.d("FAV", "Success getting crianca document: " + crianca?.nome)
                        val estrategiasPref: ArrayList<Estrategia?>? =
                            crianca?.estrategiasFavoritas
